...
predictors <- c("AGE_BIN","AGE_BIN*RIAGENDR", "RIAGENDR", "RIDRETH1", "SMQ020",
                 "HIQ011", "BPQ020", "log(BMXBMI)", "BPQ080",
                 "PAQMV", "DIDTOT","DMDBORNT",
                "INC3", "DMDEDUC2", "SDDSRVYR")
covariates <- c("AGE_BIN", "RIAGENDR", "RIDRETH1", "SMQ020",
                "HIQ011", "BPQ020", "BMXBMI", "BPQ080",
                 "PAQMV", "DIDTOT","DMDBORNT",
                "INC3", "DMDEDUC2", "SDDSRVYR","ps")
...
matched_df2 <- match.data(matched_data2) %>%
  filter(!SDMVSTRA %in% c(51, 55, 63, 57, 70, 74, 87))

# Assign treated units' weights to their matched controls (transfer weights)
matched_df2$weight_transferred <- ifelse(
  matched_df2$DEPR_NEW == 1, 
  matched_df2$MEC15YR, 
  matched_df2$MEC15YR[matched_df2$subclass]  # Transfer within subclass
)
levels(matched_df2$CAD) <- c("no","yes")



# Step 4: Analyze matched data with survey weights
matched_design2 <- svydesign(
  ids = ~SDMVPSU, 
  strata = ~SDMVSTRA, 
  weights = ~MEC15YR, 
  data = matched_df2,
...
rep2 <- as.svrepdesign(matched_design2, type="bootstrap", replicates=250)
repweights <- weights(rep2, type="analysis")
...
importance_list <- lapply(all_m2, function(mod) {
...
for (var in binary_vars) {
  ...
  ale_list[[var]] <- lapply(seq_along(all_m2), function(i) {
...
ggplot(agg_diff, aes(x = predictor)) +
  ...
  labs(x = "Predictor", y = "ALE",
       title = "Effect size of chest pain characterization on CAD prevalence:\n Depression Cohort\n SENSITIVTY ANALYSIS") +
  theme_minimal() +
  theme(
...
for (rep in 1:ncol(repweights)) {  
  ...
  all_m02[[rep]] <- rf_model
  
}

# Initialize vectors (one element per model) to store the 0.632 bootstrapped metrics
n_models <- length(all_m02)
auc_vec   <- numeric(n_models)
f1_vec    <- numeric(n_models)
gmean_vec <- numeric(n_models)
sens_vec  <- numeric(n_models)
...
# Create a list to store the importance vectors from each model
importance_list <- lapply(all_m02, function(mod) {
  imp <- mod$finalModel$variable.importance
  # If the importance is a matrix, you might be interested in one measure (e.g., MeanDecreaseGini)
  # Adjust the column name if needed.
  if(is.matrix(imp) && "MeanDecreaseGini" %in% colnames(imp)) {
...
for (var in binary_vars) {
  cat("Processing variable:", var, "\n")
  ale_list[[var]] <- lapply(seq_along(all_m02), function(i) {
    # Extract the ranger model from caret
    mod_rf <- all_m02[[i]]
    tempdata <- all_data2[[i]]
    newdata <- tempdata[tempdata$DEPR_NEW==0,]
