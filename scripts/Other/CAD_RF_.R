...
wd_subset <- wd[, c(responses, predictors, "CDQ001", "DEPR_NEW", "SDMVPSU", "SDMVSTRA", "MEC15YR", "SEQN")]
...
survey_design <- subset(design, design$variables["CDQ001"] == 1 & design$variables["DEPR_NEW"] == 1)
...
# SAVE
dep_svy <- survey_design

# create full data replicate samples
rep <- as.svrepdesign(survey_design, type="bootstrap", replicates=300)
...
tune_grid <- expand.grid(
  mtry = c(8, 12, 16),  # Try different values for mtry
...
importance_list <- lapply(all_model0, function(mod) {
...
#   partial_list[[var]] <- lapply(seq_along(all_model0), function(i) {
...
for (var in binary_vars) {
  ...
  ale_list[[var]] <- lapply(seq_along(all_model0), function(i) {
