...
unique_predictors <- unique(ale_dep$predictor)
...
for(pred in unique_predictors) {
  ...
  pred_data <- ale_dep[ale_dep$predictor == pred, ]
...
# Get unique predictors
unique_predictors <- unique(ale_nodep$predictor)
...
for(pred in unique_predictors) {
  # Subset data for current predictor
  pred_data <- ale_nodep[ale_nodep$predictor == pred, ]
...
for (i in seq_along(predictors)) {
  ...
  # Get bootstrap estimates for dep and nodep cohorts
  dep_values <- diff_distributions_dep[[predictor]]
  nodep_values <- diff_distributions_nodep[[predictor]]
  
  # Compute risk difference (dep - nodep) for each bootstrap iteration
  risk_diff <- dep_values-nodep_values
  
  # Calculate median and 95% CI of the risk difference
  median_diff <- median(risk_diff)
  lower_ci <- quantile(risk_diff, probs = 0.025)
  upper_ci <- quantile(risk_diff, probs = 0.975)
...
ggplot(comp_long, aes(x = Predictor, y = value)) +
  ...
  labs(title = "Within-cohort and Across-cohort differences in cohort-specifc pain profile ALE estimates", x = "Predictors", y = "Difference") +
