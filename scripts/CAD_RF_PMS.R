...
check_survey_balance <- function(design, treatment_var, covariates) {
  ...
  # Convert survey design to dataframe with weights
  ...
  # Compute survey-weighted median, IQR, mean, and SD for numeric covariates
  balance_table_numeric <- lapply(numeric_covars, function(x) {
    # Get survey-weighted median and IQR
    medians <- svyby(as.formula(paste("~", x)),
                     as.formula(paste("~", treatment_var)),
                     design, svyquantile, quantiles = 0.5, ci = TRUE, keep.var = FALSE)
    
    q1 <- svyby(as.formula(paste("~", x)),
                as.formula(paste("~", treatment_var)),
                design, svyquantile, quantiles = 0.25, ci = FALSE, keep.var = FALSE)
    
    q3 <- svyby(as.formula(paste("~", x)),
                as.formula(paste("~", treatment_var)),
                design, svyquantile, quantiles = 0.75, ci = FALSE, keep.var = FALSE)
    
    # Get survey-weighted mean and SD for SMD calculation
    means <- svyby(as.formula(paste("~", x)),
                   as.formula(paste("~", treatment_var)),
                   design, svymean, na.rm = TRUE)
    
    sds <- svyby(as.formula(paste("~", x)),
                 as.formula(paste("~", treatment_var)),
                 design, svyvar, na.rm = TRUE)
    
    # Calculate SMD using mean and SD
    smd <- (means[2, 2] - means[1, 2]) / sqrt((sds[2, 2] + sds[1, 2]) / 2)
    
    # Format as "Median (IQR)"
    treatment_median_iqr <- sprintf("%.2f (%.2f - %.2f)", medians[2, 2], q1[2, 2], q3[2, 2])
    control_median_iqr <- sprintf("%.2f (%.2f - %.2f)", medians[1, 2], q1[1, 2], q3[1, 2])
    
    data.frame(
      Variable = x,  # Continuous variables retain original names
      SMD = smd,
      Treatment_Mean_SD = sprintf("%.2f ± %.2f", means[2, 2], sqrt(sds[2, 2])),
      Control_Mean_SD = sprintf("%.2f ± %.2f", means[1, 2], sqrt(sds[1, 2])),
      Treatment_Median_IQR = treatment_median_iqr,
      Control_Median_IQR = control_median_iqr
    )
  }) |> dplyr::bind_rows()
  
  # Compute survey-weighted proportions for factor variables (percentage only)
  balance_table_factor <- if (length(factor_covars) > 0) {
    lapply(factor_covars, function(f) {
      lvls <- levels(df[[f]])
      
      lapply(lvls, function(l) {
        df$temp <- as.numeric(df[[f]] == l)  # Convert factor level to binary variable
        design_temp <- update(design, temp = df$temp)
        
        # Get survey-weighted proportion
        means <- svyby(~temp, as.formula(paste("~", treatment_var)), design_temp, svymean)
        
        # Calculate SMD for binary factor
        smd <- (means[2, "temp"] - means[1, "temp"]) /
          sqrt((means[1, "temp"] * (1 - means[1, "temp"]) +
                  means[2, "temp"] * (1 - means[2, "temp"])) / 2)
        
        # Format as "x%"
        treatment_pct <- sprintf("%.1f%%", means[2, "temp"] * 100)
        control_pct <- sprintf("%.1f%%", means[1, "temp"] * 100)
        
        data.frame(
          Variable = paste0(f, ":", l),  # Factor levels appended to Variable name
          SMD = smd,
          Treatment_Mean = treatment_pct,
          Control_Mean = control_pct
        )
      }) |> dplyr::bind_rows()
    }) |> dplyr::bind_rows()
  } else {
  ...
  # Combine numeric and factor balance tables
  balance_table <- dplyr::bind_rows(balance_table_numeric, balance_table_factor)
...
# Approx pop size
# Get estimated population size from survey design
population_size <- svytotal(~one, update(matched_design, one = 1))

# Print estimated population size
...
importance_list <- lapply(all_m, function(mod) {
...
for (var in binary_vars) {
  ...
  ale_list[[var]] <- lapply(seq_along(all_m), function(i) {
...
# Create a list to store the importance vectors from each model
importance_list <- lapply(all_m0, function(mod) {
  imp <- mod$finalModel$variable.importance
  # If the importance is a matrix, you might be interested in one measure (e.g., MeanDecreaseGini)
  # Adjust the column name if needed.
  if(is.matrix(imp) && "MeanDecreaseGini" %in% colnames(imp)) {
...
for (var in binary_vars) {
  cat("Processing variable:", var, "\n")
  ale_list[[var]] <- lapply(seq_along(all_m0), function(i) {
    # Extract the ranger model from caret
    mod_rf <- all_m0[[i]]
    tempdata <- all_data[[i]]
    newdata <- tempdata[tempdata$DEPR_NEW==0,]
...
# Performance metrics for Depression Model
dep_performance <- data.frame(
  Metric = c("AUC", "GMean", "Sensitivity", "Specificity", "Brier"),
  Mean = round(c(0.8126702, 0.7808932, 0.7387462, 0.8332365, 0.1305512), 3),
  Lower95 = round(c(0.7733410, 0.7144793, 0.6194474, 0.8022692, 0.1186575), 3),
  Upper95 = round(c(0.8452124, 0.8521940, 0.8577971, 0.8704126, 0.1444466), 3)
)

# Performance metrics for No Depression Model
no_dep_performance <- data.frame(
  Metric = c("AUC", "GMean", "Sensitivity", "Specificity", "Brier"),
  Mean = round(c(0.8282392, 0.7589933, 0.7009862, 0.8326829, 0.1291206), 3),
  Lower95 = round(c(0.7957906, 0.6985395, 0.6145769, 0.7988151, 0.1161535), 3),
  Upper95 = round(c(0.8560501, 0.8145354, 0.8052643, 0.8631096, 0.1432851), 3)
)

# Render tables
kable(dep_performance, format = "html") %>%
  kable_styling(full_width = FALSE, font_size = 20, position = "center") %>%
  add_header_above(c(" " = 1, "Performance Metrics - Depression Model" = 3)) %>%
  row_spec(0, bold = TRUE, color = "black", font_size = 20)

kable(no_dep_performance, format = "html") %>%
  kable_styling(full_width = FALSE, font_size = 20, position = "center") %>%
  add_header_above(c(" " = 1, "Performance Metrics - No Depression Model" = 3)) %>%
  row_spec(0, bold = TRUE, color = "black", font_size = 20)